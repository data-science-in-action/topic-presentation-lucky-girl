...
\end{align}
在独立同分布白噪声假设下近似服从$\chi^2(m)$分布，当$Q(m)>q\chi^2(1-\alpha,m)$时，拒绝$H_0$,认为该序列不是白噪声序列,存在异方差性，即存在ARCH效应。
...
\end{align}
其中$\bar{\omega}=\frac{1}{T}\sum_{t=1}^{T}\mu_t^2$是{$\mu^2}的样本均值。令
\begin{align}
...
该方法下，原假设为不存在异方差性，因此，当LM值较大，p值较小时，拒绝原假设，说明有ARCH效应。
